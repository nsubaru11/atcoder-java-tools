...
// @license        Apache-2.0
// @homepageURL    https://github.com/nsubaru11/atcoder-java-tools/tree/main/userscripts/AtCoderListingTasks
...
// @updateURL      https://raw.githubusercontent.com/nsubaru11/atcoder-java-tools/main/userscripts/AtCoderListingTasks/dist/AtCoderListingTasks.user.js
// @downloadURL    https://raw.githubusercontent.com/nsubaru11/atcoder-java-tools/main/userscripts/AtCoderListingTasks/dist/AtCoderListingTasks.user.js
// ==/UserScript==
...
(() => {
	...
	`;
		...
		const STORE_INFO = [
			{ storeName: STORE_NAME.option, keyPath: "name" },
			{
				storeName: STORE_NAME.problemList,
				keyPath: "contestName",
			},
		];
		const REMOVE_INTERVAL = 60 * 60 * 1000;
		const REMOVE_BASE = 10 * 24 * 60 * 60 * 1000;
		const ACCESS_INTERVAL = 5 * 60 * 1000;
			...
			loadData: async function () {
				if (this.dbExists) {
					...
					if (resArray[2] !== null) {
						...
						this.lastRemove = now;
						setTasks.push(
							this.db.setData(STORE_NAME.option, { name: "lastRemove", value: this.lastRemove }),
						);
					}
					if (resArray[3] !== null) {
					...
					if (resArray[0] !== null) {
						...
						if (now - resArray[0].lastAccess >= ACCESS_INTERVAL) {
							setTasks.push(
								this.db.setData(STORE_NAME.problemList, {
		...
		Launcher.prototype = {
			...
			addList: function () {
				...
				label.css("color", task_table.css("color"));
			...
			addModal: function () {
				...
				this.isAll =
					this.setting.atOnce.begin === 0 && this.setting.atOnce.end === this.setting.problemList.length - 1;
				...
				begin_button.html(
					`${this.setting.problemList[this.setting.atOnce.begin].diff}<span class="caret ${PRE}-caret"></span>`,
				);
				end_button.html(
					`${this.setting.problemList[this.setting.atOnce.end].diff}<span class="caret ${PRE}-caret"></span>`,
				);
				...
				let open = $("<button>", {
					...
					text: TEXT.atOnce[this.setting.lang],
				});
				open.on("click", () => {
					...
					if (this.setting.contestCategory !== "other") {
						...
						if (this.isAll) {
							...
							this.setting.atOnceSetting[this.setting.contestCategory].end =
								this.setting.problemList.length - 1;
						} else {

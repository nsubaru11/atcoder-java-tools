...
export function printUsage(): void {
	...
	console.error("  --full                     test/localtest/submit/codecompare: WA 差分を折りたたまず全行表示");
	console.error("  --wa-only                  test/localtest/submit/codecompare: WA 差分のうち不一致行だけ抽出（行番号は保持）");
	console.error("  --max-lines=N              test/localtest/submit/codecompare: WA 差分の折りたたみ行数を変更（既定: 20）");
}
